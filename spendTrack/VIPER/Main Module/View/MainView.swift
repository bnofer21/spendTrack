...
class MainView: InfoView {
    ...
    internal override func updateViews() {
        DispatchQueue.main.async { [weak self] in
            if let model = self?.model as? InfoPriceModel {
                self?.btcPriceLabel.text = model.price
            } else if let model = self?.model as? InfoBalanceModel {
                self?.balanceLabel.text = model.currentbalance
                self?.spendStats[0].text = "All time spendings: \(model.allTimeSpent)"
                self?.spendStats[1].text = "Today spendings spendings: \(model.todaySpent)"
            }
        }
    }
